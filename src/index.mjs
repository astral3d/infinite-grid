...
export class InfiniteGrid{
    ...
    recomputeLL(){
        ...
        this.coordinateBounds = [
            this.ll.x * this.size.x, 
            this.ur.y * this.size.y, 
            this.ur.x * this.size.x, 
            this.ll.y * this.size.y
        ];
    }
        ...
        for(let row = -1 * this.cellRadius; row <= this.cellRadius; row++){
            for(let col = -1 * this.cellRadius; col <= this.cellRadius; col++){
                cell = oldCells.find((cell)=>{
                    return (
                        cell.options.x === row + this.offset.x &&
                        cell.options.y === col + this.offset.y 
                    );
                });
                if(cell){
    ...
    checkForTransition(){
        ...
        if(this.avatar){
            ...
            if(
                this.avatar.position.x <= this.centerCoordinateBounds[0]
            ){
                transition.x--;
            }
            if(
                this.avatar.position.y >= this.centerCoordinateBounds[1] 
            ){
                transition.y++;
            }
            if(
                this.avatar.position.x >= this.centerCoordinateBounds[2]
            ){
                transition.x++;
            }
            if(
                this.avatar.position.y >= this.centerCoordinateBounds[3]
            ){
                transition.y--;
            }
    ...
    checkForMarkerRemoval(){
        ...
        for(let lcv=0; lcv < this.markers.length; lcv++){
            if(
                this.avatar.position.x <= this.bounds[0] ||
                this.avatar.position.y <= this.bounds[1] ||
                this.avatar.position.x >= this.bounds[2] ||
                this.avatar.position.y >= this.bounds[3]
            ){
...
export class InfiniteGridMarker{
    constructor(options={}){
        this.options = options;
    }
